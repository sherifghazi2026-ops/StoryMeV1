...
const YoyaGameV4 = ({ navigation, route }) => {
  const [gameState, setGameState] = useState('home');
  const [platforms, setPlatforms] = useState([]);
  const [gems, setGems] = useState([]);
  const [books, setBooks] = useState([]);
  const [hearts, setHearts] = useState([]);
  const [fruits, setFruits] = useState([]); // فواكه وخضروات بدلاً من العقبات
  ...
  const [currentFruitInfo, setCurrentFruitInfo] = useState(null);
  ...
  const rocketSpawned = useRef(false);
  ...
  const rocketImage = require('../assets/Rocket.gif');

  // معلومات الفواكه والخضروات
  const fruitItems = [
    {
      id: 'carrot',
      emoji: '🥕',
      name: 'جزر',
      benefits: [
        'يُقَوِّي النَّظَرَ وَيُحَسِّنُ الرُّؤْيَةَ',
        'يُعَزِّزُ صِحَّةَ الْجِلْدِ وَالْبَشَرَةِ',
        'يَحْتَوِي عَلَى فِيتَامِينِ أَ الَّذِي يُقَوِّي الْمَنَاعَةَ'
      ]
    },
    {
      id: 'apple',
      ...
      name: 'تفاح',
      benefits: [
        'يُقَلِّلُ مِنَ الْكُولِيسْتِرُولِ الضَّارِ',
        'يُعَزِّزُ صِحَّةَ الْقَلْبِ',
        'يَحْتَوِي عَلَى أَلِيَافٍ تُسَاعِدُ فِي الْهَضْمِ'
      ]
    },
    {
      id: 'banana',
      ...
      name: 'موز',
      benefits: [
        'يُعْطِي طَاقَةً وَيُقَلِّلُ التَّعَبِ',
        'يُحَسِّنُ الْمِزَاجَ وَيُقَلِّلُ الْكآبَةِ',
        'غَنِيٌّ بِالْبُوتَاسِيُومِ الَّذِي يُقَوِّي الْعِظَامَ'
      ]
    },
    {
      id: 'broccoli',
      ...
      name: 'بروكلي',
      benefits: [
        'يُقَوِّي الْعِظَامَ وَيُحَافِظُ عَلَى صِحَّتِهَا',
        'يُقَلِّلُ مِنْ خُطُورَةِ الْأَمْرَاضِ',
        'يَحْتَوِي عَلَى فِيتَامِينِ سِي الَّذِي يُقَوِّي الْمَنَاعَةَ'
      ]
    }
  ];

  const bookQuestionsV4 = [
    {
  ...
  const activateRocket = () => {
    ...
    setRocketTime(5); // 5 ثواني للصاروخ
    speak("مُبَارَك! لَقَدْ حَصَلْتَ عَلَى صَارُوخٍ! سَتَطِيرُ لِخَمْسِ ثَوَانٍ!");
  ...
  const deactivateRocket = () => {
    setRocketActive(false);
    setRocketTime(0);
    speak("انْتَهَى وَقْتُ الصَّارُوخِ. أَحْسَنْتَ الْقِيَادَةَ!");
  };

  const handleBackPress = () => {
    if (gameState === 'playing' && !showQuestion && !showFruitInfo) {
      setGamePaused(true);
      return true;
    }
    return false;
  ...
  const initGame = () => {
    let nPlats = [], nGems = [], nBooks = [], nHearts = [], nFruits = [], nCoins = [];
    
    nPlats.push({ x: 0, y: SCREEN_HEIGHT - 100, width: SCREEN_WIDTH });

    for (let i = 1; i < 150; i++) { // تقليل عدد المنصات لجعل اللعبة أسهل
      ...
      // تجنب وضع الجواهر والفواكه في نفس المكان
      if (Math.random() > 0.7) {
        nGems.push({ 
          x: px + 35, 
          y: py - 50, 
      ...
      // إضافة فواكه بدلاً من العقبات
      if (Math.random() > 0.8) {
        const fruitType = fruitItems[Math.floor(Math.random() * fruitItems.length)];
        nFruits.push({
          x: px + 20,
          y: py - 50,
          id: 'f'+i,
      ...
      if (!rocketSpawned.current && i === 30) {
        ...
        rocketSpawned.current = true;
      }
      
      // توزيع الكتب بمسافات متساوية
      if (i % 25 === 0 && nBooks.length < bookQuestionsV4.length) {
        nBooks.push({ 
          ...bookQuestionsV4[nBooks.length], 
          x: px + 35, 
          y: py - 70, 
          collected: false, 
          id: 'b'+i 
    ...
    setRocketActive(false);
    setRocketTime(0);
    setShowFruitInfo(false);
    
    heartSpawned.current = false;
    rocketSpawned.current = false;
    scrollOffset.current = 0;
    pos.current = { x: SCREEN_WIDTH/2 - HERO_WIDTH/2, y: SCREEN_HEIGHT - 200 };
    vel.current = { x: 0, y: 0 };
    
    speak("مَرْحَبًا يَا " + userName + "! هَذَا هُوَ الْمُسْتَوَى الرَّابِعُ. لِنَبْدَأْ مُغَامَرَتَنَا الْجَدِيدَةَ!");
  ...
  const update = () => {
    ...
    const progress = Math.min((scrollOffset.current / 25000) * 100, 100);
    setLevelProgress(progress);
    
    const heroScreenY = pos.current.y + scrollOffset.current;
    if (heroScreenY < 80) scrollOffset.current += (80 - heroScreenY);
    
    // تأثير الصاروخ: تقليل الجاذبية عندما يكون الصاروخ نشطاً
    if (rocketActive) {
      vel.current.y = -8; // يطير للأعلى
      vel.current.x *= 1.2; // زيادة السرعة الأفقية
    } else {
    ...
    if (vel.current.y > 0 && !rocketActive) {
    ...
    // جمع الفواكه
    fruits.forEach(f => {
      if (!f.collected && Math.abs(pos.current.x - f.x) < 40 && Math.abs(pos.current.y - f.y) < 40) {
        f.collected = true;
        setScore(s => s + f.value);
        setCurrentFruitInfo(f.type);
        setShowFruitInfo(true);
        hapticFeedback('success');
        
        const fruitName = f.type.name;
        const randomBenefit = f.type.benefits[Math.floor(Math.random() * f.type.benefits.length)];
        speak(`أَحْسَنْتَ! وَجَدْتَ ${fruitName}. ${randomBenefit}`);
      }
    });
    ...
    // التحقق من خروج اللاعب من الشاشة
  ...
  useEffect(() => {
    if (gameState === 'playing' && !showQuestion && !showWinScreen && !gamePaused && !showFruitInfo) {
      gameLoopRef.current = setInterval(update, 16);
    } else {
      clearInterval(gameLoopRef.current);
    }
    return () => clearInterval(gameLoopRef.current);
  }, [gameState, showQuestion, showWinScreen, gamePaused, showFruitInfo]);
  ...
  const handleAnswer = (opt) => {
    ...
    if (opt.isCorrect) {
      ...
      setCollectedBooks(c => {
        const newCount = c + 1;
        if (newCount >= bookQuestionsV4.length) {
          setTimeout(() => {
            setShowWinScreen(true);
            confettiRef.current && confettiRef.current.start();
            const skillsText = bookQuestionsV4.map(q => q.summary).join("، وَ");
            speak("رَائِعٌ يَا " + userName + "! لَقَدْ أَنْهَيْتَ الْمُسْتَوَى الرَّابِعَ. " + skillsText);
          }, 1500);
        }
        return newCount;
      });
  ...
  const shareGame = () => {
    const info = bookQuestionsV4.map(q => q.summary).join("\n- ");
    Share.share({
      message: `أنا البطل ${userName}، أنهيت المستوى الرابع في يويَا! 🎮\n\nما تعلمته:\n- ${info}\n\nجمعت ${score} نقطة!`,
  ...
  const joystickResponder = useRef(PanResponder.create({
    ...
    onPanResponderMove: (_, gs) => {
      ...
      vel.current.x = (moveX / 50) * (rocketActive ? 10 : 7.5); // زيادة السرعة مع الصاروخ
  ...
  const renderFruitInfo = () => (
    <Modal visible={showFruitInfo} transparent animationType="fade">
      <View style={styles.fruitOverlay}>
        <View style={styles.fruitCard}>
          <Text style={styles.fruitEmoji}>{currentFruitInfo?.emoji}</Text>
          <Text style={styles.fruitTitle}>مَعْلُومَةٌ عَنْ {currentFruitInfo?.name}</Text>
          
          <ScrollView style={styles.benefitsScroll}>
            {currentFruitInfo?.benefits.map((benefit, index) => (
          ...
          <TouchableOpacity 
            ...
            onPress={() => {
              setShowFruitInfo(false);
              setCurrentFruitInfo(null);
            }}
          >
            <Text style={styles.continueBtnText}>⚡ اِسْتَمِرْ فِي اللَّعِبِ</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
    ...
    <View style={styles.container}>
      {gameState === 'playing' ? (
        showWinScreen ? renderWinScreen() : (
          <View style={styles.gameArea}>
            ...
            <View style={styles.hud}>
              ...
              <View style={styles.hudItem}>
                <Text style={styles.hudText}>📚 {collectedBooks}/{bookQuestionsV4.length}</Text>
              </View>
              <View style={styles.hudItem}>
              ...
              {rocketActive && (
                <View style={styles.hudItem}>
                  <Text style={[styles.hudText, {color: '#FF9F43'}]}>🚀 {rocketTime}</Text>
                </View>
              )}
            </View>
            ...
            <Animated.View style={{ flex: 1, transform: [{ translateY: scrollAnim }] }}>
              ...
              {gems.map(g => !g.collected && (
                <Animated.Text 
                  ...
                  style={[
                    ...
                    g.type === 'rocket' && styles.rocketItem,
                    { left: g.x, top: g.y }
                  ]}
                >
                  {g.type === 'rocket' ? '🚀' : g.type === 'rare' ? '💎✨' : '💎'}
                </Animated.Text>
              ))}
              ...
              {fruits.map(f => !f.collected && (
                <Text 
                  key={f.id} 
                  style={[styles.item, styles.fruitItem, { left: f.x, top: f.y }]}
                >
                  {f.type.emoji}
                </Text>
              ))}
              ...
              >
                {rocketActive ? (
                  <Animated.View style={[
                    ...
                    <Image source={rocketImage} style={styles.heroImg} />
                  </Animated.View>
                ) : (
                  <Image 
              ...
              <View style={styles.modalOverlay}>
                <View style={styles.modalCard}>
                  <View style={styles.questionHeader}>
                    ...
                    {currentQuestionData?.id === 6 && (
                      <Text style={styles.timeWarning}>⏱️ وقت قصير!</Text>
                    )}
                  </View>
        ...
        <View style={styles.menu}>
          ...
          <Text style={styles.menuTitle}>المستوى 4</Text>
          <Text style={styles.welcomeName}>أَهْلًا {userName} 👋</Text>
          <Text style={styles.menuSubtitle}>مغامرة تعليمية مع الفواكه الصحية!</Text>
...
const styles = StyleSheet.create({
  ...
  hudText: { fontSize: 20, fontWeight: 'bold', color: '#2C3E50' },
  comboText: {
    fontSize: 12,
    color: '#FF9F43',
    fontWeight: 'bold',
